...
export function Island({ isRotating, setIsRotating, ...props }) {
  ...
  useFrame(() => {
    if (!isRotating) {
       ...
       * 2. (rotation % 2 * Math.PI)) + 2 * Math.PI adds 2 * Math.PI to the result from step 1. This is done to ensure that the value remains positive and within the range of 0 to 2 * Math.PI even if it was negative after modulo operation in step 1.
